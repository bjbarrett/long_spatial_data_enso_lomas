...
// 
//     for ( i in 1:N ) {
//       lambda[i] = v_mu[season_index[i] ]  + v[group_index[i] , season_index[i] ] + // means per seas and varef per season per group
//        (v_mu[2+season_index[i]] + v[group_index[i], 2+season_index[i]]) * am_pred[year_index[i],season_index[i]] ; //
//       lambda[i] = exp(lambda[i]);
//   }
//   hr_area_obs ~ gamma( lambda/k , 1/k );
// }
